...
function stepResultSummary(result) {
  ...
  const parts = []
  // agent_plan step
  if (result.plan_steps != null)             parts.push(`${result.plan_steps}-step plan`)
  if (result.key_decision != null)           parts.push(result.key_decision.slice(0, 50))
  // model_decision step
  ...
  return (
    <div style={{ padding: '2rem', overflowY: 'auto', height: '100%' }}>
      ...
      <div style={{ marginBottom: '2rem', paddingBottom: '1.5rem', borderBottom: '1px solid var(--border)' }}>
        ...
        {/* Agent Plan — model-generated before execution */}
        {run.agent_plan && !run.agent_plan.fallback && (
          <div style={{ marginBottom: '14px', padding: '12px 14px', background: 'rgba(67,56,202,0.04)', border: '1px solid rgba(67,56,202,0.15)', borderRadius: 'var(--radius)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', fontWeight: 600, color: 'var(--accent)', letterSpacing: '0.06em' }}>
                AGENT PLAN
              </p>
              <span style={{ fontFamily: 'var(--font-sans)', fontSize: '10px', color: 'var(--text-3)' }}>
                generated by {run.agent_plan.model || 'Gemini Flash'} before execution
              </span>
            </div>
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '12px', color: 'var(--text-2)', lineHeight: 1.55, marginBottom: '8px' }}>
              {run.agent_plan.reasoning}
            </p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
              {(run.agent_plan.steps || []).map((step, i) => (
                <span key={i} style={{
                  fontFamily: 'var(--font-mono)', fontSize: '10px',
                  padding: '2px 7px',
                  background: 'rgba(67,56,202,0.08)', color: 'var(--accent)',
                  border: '1px solid rgba(67,56,202,0.2)', borderRadius: '3px',
                }}>
                  {i + 1}. {step.replace(/_/g, ' ')}
                </span>
              ))}
            </div>
            {run.agent_plan.key_decision && (
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: '11px', color: 'var(--text-3)', marginTop: '8px' }}>
                Key decision: {run.agent_plan.key_decision}
              </p>
            )}
          </div>
        )}
      ...
      {run.model_decision && (
        <div style={{ marginBottom: '2rem' }}>
          ...
          {/* Rejected tools — explicit model rejections */}
          {run.result?.tool_selection?.rejected_tools?.length > 0 && (
            <div style={{ marginTop: '8px', padding: '10px 14px', background: 'rgba(220,38,38,0.04)', borderRadius: 'var(--radius-sm)', border: '1px solid rgba(220,38,38,0.12)' }}>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', fontWeight: 700, color: '#DC2626', letterSpacing: '0.08em', marginBottom: '6px' }}>
                EXPLICITLY REJECTED TOOLS
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                {run.result.tool_selection.rejected_tools.map((tool, i) => (
                  <div key={i} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: '#DC2626', flexShrink: 0 }}>✗</span>
                    <span style={{ fontFamily: 'var(--font-sans)', fontSize: '11px', color: 'var(--text-3)', textDecoration: 'line-through' }}>{tool}</span>
                  </div>
                ))}
              </div>
              {run.result.tool_selection.reasoning && (
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: '10px', color: 'var(--text-3)', marginTop: '6px', fontStyle: 'italic' }}>
                  {run.result.tool_selection.reasoning}
                </p>
              )}
            </div>
                ...
                <div key={stepId}>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
                      {(() => {
                        const isModelStep = ['agent_plan','model_decision','tool_selection','case_selection','evidence_sufficiency','challenge_review'].includes(step.id)
                      ...
                      <span style={{
                        ...
                        color: ['agent_plan','model_decision','tool_selection','case_selection','evidence_sufficiency','challenge_review'].includes(step.id) ? 'var(--accent)' : step.result?.skipped ? 'var(--text-3)' : 'var(--text)',
                        fontWeight: ['agent_plan','model_decision','tool_selection','case_selection','evidence_sufficiency','challenge_review'].includes(step.id) ? 600 : 500,
